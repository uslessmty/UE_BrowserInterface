...
void SLocalBrowser::Construct( const FArguments& InArgs )
{
	...
	TSharedPtr<SViewport> GameViewportWidget = SNew(SViewport)
				// 下面对视口进行一些设置，比如是否启用Gamma校正，这个根据你的需求来
				.EnableGammaCorrection(false);
	ChildSlot
	[
		SAssignNew(BrowserView, SWebBrowserView, BrowserWindow) // 原有的SWebBrowserView配置
			...
			.OnBeforePopup(this, &SLocalBrowser::HandleBeforePopup)
			.OnCreateWindow(this, &SLocalBrowser::HandleCreateWindow)
			.OnCloseWindow(this, &SLocalBrowser::HandleCloseWindow)
			.OnBeforeNavigation(OnBeforeNavigation)
			.OnLoadUrl(OnLoadUrl)
			.OnShowDialog(OnShowDialog)
			.OnDismissAllDialogs(OnDismissAllDialogs)
			.Visibility(this, &SLocalBrowser::GetViewportVisibility)
			.OnSuppressContextMenu(this, &SLocalBrowser::HandleSuppressContextMenu)
			
	];
}

void SLocalBrowser::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
EVisibility SLocalBrowser::GetViewportVisibility() const
{
...
bool SLocalBrowser::HandleBeforePopup( FString URL, FString Frame )
{
#if UE_BUILD_DEVELOPMENT || UE_BUILD_DEBUG
...
bool SLocalBrowser::HandleSuppressContextMenu()
{
	return true;
}

bool SLocalBrowser::HandleCreateWindow( const TWeakPtr<IWebBrowserWindow>& NewBrowserWindow, const TWeakPtr<IWebBrowserPopupFeatures>& PopupFeatures )
{
#if UE_BUILD_DEVELOPMENT || UE_BUILD_DEBUG
...
bool SLocalBrowser::HandleCloseWindow( const TWeakPtr<IWebBrowserWindow>& BrowserWindowPtr )
{
#if UE_BUILD_DEVELOPMENT || UE_BUILD_DEBUG
...
int32 SLocalBrowser::GetTextureWidth() const
{
	if ( !BrowserWindow.IsValid() )
		return 0;
...
int32 SLocalBrowser::GetTextureHeight() const
{
	if ( !BrowserWindow.IsValid() )
		return 0;
...
FColor SLocalBrowser::ReadTexturePixel( int32 X, int32 Y ) const
{
...
TArray<FColor> SLocalBrowser::ReadTexturePixels( int32 X, int32 Y, int32 Width, int32 Height ) const
{
...
void SLocalBrowser::LoadURL( FString NewURL )
{
	if ( BrowserView.IsValid() )
		BrowserView->LoadURL( NewURL );
}

void SLocalBrowser::LoadString( FString Contents, FString DummyURL )
{
	if ( BrowserView.IsValid() )
		BrowserView->LoadString( Contents, DummyURL );
}

void SLocalBrowser::Reload()
{
	if ( BrowserView.IsValid() )
		BrowserView->Reload();
}

void SLocalBrowser::StopLoad()
{
	if ( BrowserView.IsValid() )
		BrowserView->StopLoad();
}

FString SLocalBrowser::GetUrl() const
{
	if ( BrowserView.IsValid() )
...
bool SLocalBrowser::IsLoaded() const
{
	if ( BrowserView.IsValid() )
...
bool SLocalBrowser::IsLoading() const
{
	if ( BrowserView.IsValid() )
...
void SLocalBrowser::ExecuteJavascript( const FString& ScriptText )
{
	if ( BrowserView.IsValid() )
		BrowserView->ExecuteJavascript( ScriptText );
}

void SLocalBrowser::BindUObject( const FString& Name, UObject* Object, bool bIsPermanent )
{
	if ( BrowserView.IsValid() )
	{
		BrowserView->BindUObject( Name, Object, bIsPermanent );
		UE_LOG(LogTemp, Warning, TEXT("Browser Binding Object"));
	}
}

void SLocalBrowser::UnbindUObject( const FString& Name, UObject* Object, bool bIsPermanent )
{
	if ( BrowserView.IsValid() )
	{
		BrowserView->UnbindUObject( Name, Object, bIsPermanent );
		UE_LOG(LogTemp, Warning, TEXT("Browser Unbinding Object"));
	}
}
